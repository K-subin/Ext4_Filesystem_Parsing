...
def GDT(log_block_size):
    f.seek(GDT_addr * sector)
    GDT = f.read(sector)
...
def Directory_Entry(DP_addr):
    ...
    File_name, Folder_name, Block_Group_num = [], [], []
    i = 24
    while(i < DE_size):
        # Inode_num
        ...
        if file_type == 'Directory' and name != 'lost+found':
            Folder_name.append(name_hex.decode())
            Block_Group_num.append(int((Inode_num-1)/Inode_Per_Group))
        
        i += record_len

    return File_name, Folder_name, Block_Group_num


def folder_GDT(log_block_size, bg_num):
    bg_size = 32
    ...
    GDT_str = binascii.b2a_hex(GDT).decode() 
    GDT_list = [GDT_str[i:i+8] for i in range(0, bg_size*2, 8)]
...
log_block_size, Inode_Per_Group = Superblock(LBA)
GDT_addr = Superblock_addr - 2 + log_block_size * 2

Inode_Table_addr = GDT(log_block_size)

DP_addr = Inode_Table(Inode_Table_addr, 2) # root directory

File_name, Folder_name, Block_Group_num = [], [], []
File_name, Folder_name, Block_Group_num = Directory_Entry(DP_addr)
...
while(len(Folder_name) != 0):
    ...
    for bg_num in Block_Group_num:
        f_Inode_Table_addr = folder_GDT(log_block_size, bg_num)
        
        f_DP_addr = Inode_Table(f_Inode_Table_addr, 1)

        f_File_name, f_Folder_name, f_Block_Group_num = [], [], []
        f_File_name, f_Folder_name, f_Block_Group_num = Directory_Entry(f_DP_addr)

        print(Folder_name[i], '-->', f_File_name)
        i+=1

    Block_Group_num, Folder_name = f_Block_Group_num, f_Folder_name